...
public static class WebhookSubscriptionExtensions
{
    ...
    public static bool IsActive(this WebhookSubscription subscription)
    {
        ...
        return subscription.Active && subscription.CallbackUrl != null;
    }
    ...
    /// </summary>
    /// <param name="subscription">The webhook subscription.</param>
    /// <returns>The success rate percentage, or 0 if no deliveries have been attempted.</returns>
    ...
    public static int GetNextRetryDelay(this WebhookSubscription subscription, int currentRetryCount)
    {
        ...
        if (subscription.RetryPolicy == null || currentRetryCount < 0)
        {
            return subscription.RetryPolicy?.InitialDelayMs ?? 1000;
        }

        // Exponential backoff with jitter
        var delay = Math.Min(
            (subscription.RetryPolicy?.InitialDelayMs ?? 1000) * (int)Math.Pow(2, currentRetryCount),
            subscription.RetryPolicy?.MaxDelayMs ?? 60000
        );

        // Add jitter: ±20% random variation
        var random = new Random(subscription.Id.GetHashCode() ^ currentRetryCount);
